            ...
            {categories.map((category) => (
              <div key={category.name}>
                <div
                  ...
                  onClick={() => setOpenDropdown(openDropdown === category.name ? '' : category.name)}
                >
                  <span className="font-semibold">{category.name}</span>
                  <ExpandMore className={`transition-transform ${openDropdown === category.name ? 'rotate-180' : ''}`} />
                </div>
                {openDropdown === category.name && (
                  <div className="pl-4">
                    {category.models.map((model) => (
                      <FormControlLabel
                        key={model}
                        control={<Checkbox checked={selectedModel === model} onChange={() => setSelectedModel(model)} />}
                        label={model}
                      />
                    ))}
                  </div>
